CITATIONS: 

Libraries & Fonts
D3.js: Bostock, M. (2023). D3: Data-Driven Documents (Version 7). Observable. https://d3js.org
Google Fonts – Inter: Rasmus, A. (2016). Inter [Typeface]. Google Fonts. https://fonts.google.com/specimen/Inter
Google Fonts – Oswald: Vernon Adams et al. (2011). Oswald [Typeface]. Google Fonts. https://fonts.google.com/specimen/Oswald

Datasets 
https://moneypuck.com/data.htm

Custom canucks: https://sweezy-cursors.com/cursor/vancouver-canucks-stick-animated/ 
Implementing cursor: https://medium.com/@benzbraunstein/using-javascript-and-css-to-make-custom-cursors-dd75d40520

------------------------------------------------------------------------------------------------------------------------------

HOW TO USE THE WEBSITE:
Navigation:
A column of dots on the right side of the screen acts as the navigation menu. 
Each dot corresponds to a section, hover over a dot to see its label, 
and click it to jump directly to that section. You can also simply scroll through the page top to bottom.

Interactive Elements:

Quiz (Visualisation 01): Before the top scorers chart is revealed, you are asked to guess who the Canucks 
top goal scorer is over five seasons. Select a player card and click "Reveal the answer" to see if you were 
right. The full bar chart appears after you submit.

Ideal Zone Guess (Visualisation 02): Click and drag a box anywhere on the scatterplot to place your guess 
for where the "ideal zone" of team performance sits. After drawing your box, click "Confirm guess" to see 
how close you were, or "Redo" to try again.

Tooltips: Hovering over any data point across all four charts reveals a tooltip with the exact values for that point.
